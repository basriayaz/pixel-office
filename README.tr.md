...
Her ofisin bir de **teması** var (`"theme": "default" | "football" | "fashion" | "gothic" | "music" | "travel"`, çizilmiş önizlemelerden görsel olarak seçilir) — aynı oda planı, farklı duvar/zemin/eşya/oda adları (örn. skorbordlu, taktik odalı, tribünlü bir futbol kulübü; kesim masalı, mankenli, askılıklı, prova odalı bir moda atölyesi; vitraylı, meşaleli, kazanlı, zırhlı, mahzenli bir gotik malikâne; YouTube kanalı için play-düğmesi plaketli, ON AIR tabelalı, miks masalı, bateri ve gitarlı, kameralı, plak halılı bir müzik stüdyosu; dünya haritalı, kalkış panolu, dünya saatli, küreli, harita masalı, şemsiyeli plaj lounge'lu, pusula halılı, bagaj odalı, check-in kiosklu bir seyahat acentesi). Ofisler sekmelerin yanındaki ⚙ düğmesinden yönetilir: ekle (ad, Finder penceresinden seçilen çalışma klasörü, tema), yeniden adlandır, tema/klasör değiştir, sil (yalnızca boşken); değişiklikler `config.json`'a yazılır ve anında uygulanır.
